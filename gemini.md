...
- **2026-07-08**: **Frete por CEP na página do combo** + oferta "frete grátis hoje". Endpoint `api/_combo-frete.js` via `render.js ?tipo=frete` (ViaCEP + tabela `regras_frete` + `calcularFrete` por peso). Campo `peso_kg` nos 6 ergômetros (`ergoCatalog.js`) + no editor. Página: CEP→loading→frete real riscado→FRETE GRÁTIS + urgência + contador até 23:59 + CTA WhatsApp com o CEP. Sem função nova.
- **2026-07-08**: **Ergômetros editáveis + mídia**. Catálogo dos 6 ergômetros com override no banco (`ergo-catalog` em `landing_pages_config`, `mergeCatalog`). Aba admin `Produtos Ergômetros` (`ProdutosErgoTab.jsx`) edita textos/preços + upload de 4 fotos (compressão) e 1 vídeo p/ bucket Storage `ergo-media` (público). Página do combo com galeria + lightbox (foto tela cheia) + modal de vídeo (YouTube/Vimeo/Drive/MP4). API lê via `api/_ergo-fetch.js`. Migration obrigatória: `supabase/migrations/20260708_ergo_media_bucket.sql` (bucket + policies). `ergo-catalog`/`ergo-combos` filtrados da lista de LPs.
- **2026-07-08**: **Combos de Ergômetros** — páginas dinâmicas por conjunto de produtos. Catálogo único `src/data/ergoCatalog.js` (6 ergômetros). Página server-side `/lp/ergo/{aliases}?d={desc}` e imagem OG dinâmica `/lp/ergo/og/{slug}.png` — ambos via `api/render.js` (`?tipo=combo` / `?tipo=combo-og` → `_render-combo.js` / `_og-combo.js` com `@vercel/og`; assets em `_og-assets.js`). **Zero função nova**. Admin: aba `Combos Ergômetros` (`ComboErgoTab.jsx`) com gerador + salvar (linha `ergo-combos` em `landing_pages_config`, filtrada da lista de LPs). Rotas `/lp/ergo/*` no `vercel.json`. Editar preços/Escada em `src/data/ergoCatalog.js`.
- **2026-07-08**: **LP CrossFit editável no admin**. Config movida p/ `src/data/lpCrossfitConfig.js` (modelo plano: `produtos[]` com `categoria` + `preco_avista_txt`/`preco_prazo_txt`/`preco_nota`). `LpCrossfit.jsx` agrupa por categoria e esconde rascunho vazio. `LandingPagesTab.jsx` ganhou campo Categoria + bloco Preço em texto no form de produto. Registro `crossfit-box` criado em `landing_pages_config` (`tools/seed-crossfit-lp.mjs`, RLS permite insert anon). Cache de prévia do WhatsApp resolve-se via FB Sharing Debugger (Scrape Again) ou `?v=` no link.
- **2026-07-08**: **Nova LP Box de CrossFit** (`/lp/crossfit`, `src/pages/LpCrossfit.jsx`, rota em `main.jsx`). Baseada no catálogo Brave 2026, 6 categorias com preços (sem Ergômetros). Config por `categorias[]` (preços como string) com override Supabase `landing_pages_config` id `crossfit-box`. Card OG `public/og/crossfit.png` + casco + rota `vercel.json` + miniatura no admin. WhatsApp default (14) 98145-1119.
