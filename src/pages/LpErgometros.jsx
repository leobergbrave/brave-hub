import { Check, ChevronRight, MessageCircle, Instagram, Facebook, Youtube, Mail } from 'lucide-react';

const WA_NUMBER = '554199999999'; // ← ATUALIZE COM O NÚMERO REAL

const wa = (msg) =>
  `https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(msg)}`;

const MSG_GERAL = 'Olá! Vi os ergômetros da Brave e gostaria de solicitar uma cotação.';

const PILARES = [
  {
    emoji: '⚡',
    titulo: 'Inovação, Qualidade e Durabilidade',
    desc: 'Ergômetros desenvolvidos para suportar os treinos mais intensos, projetados com tecnologia de ponta.',
  },
  {
    emoji: '🏆',
    titulo: 'O Melhor Suporte do Mercado',
    desc: 'Consultoria especializada para garantir os melhores equipamentos para o seu espaço. Especialistas em Box de CrossFit, Studios e Academias.',
  },
  {
    emoji: '💰',
    titulo: 'Custo-Benefício',
    desc: 'O melhor custo-benefício do mercado, com preços diferenciados e condições especiais de pagamento.',
  },
];
...
const PRODUTOS = [
  {
    ...
    tagline: 'Força, resistência e resultados imediatos.',
    emoji: '🚣',
    alias: 'remo',
    features: [
      'Painel exclusivo com métricas confiáveis',
      'Puxador anatômico no padrão C2',
      'Partes em plástico resistentes',
      'Rolamentos em tecnil',
    ],
  },
  {
    nome: 'Esteira Curva Pro',
    badge: 'Pronta Entrega',
    badgeCls: 'bg-orange-500',
    tagline: 'O que era bom ficou ainda melhor.',
    emoji: '🏃',
    alias: 'estcv',
    features: [
      'Mais de 300 unidades vendidas',
      'Projeto customizado sem alavanca de tensão',
      'Durabilidade comprovada, mecânica resistente',
      'Não faz uso de energia elétrica',
      'Painel com todas as métricas de trabalho',
      'Lotes semanais, consulte disponibilidade',
    ],
  },
  {
    nome: 'Bike Erg Brave',
    badge: 'Pronta Entrega',
    badgeCls: 'bg-orange-500',
    tagline: 'Performance e endurance no padrão Concept.',
    emoji: '🚴',
    alias: 'bikeerg',
    features: [
      'Padrão similar ao Concept',
      'Painel com sensor p1, trabalho em metragem confiável',
      'Excelente para trabalho de endurance',
      'Pronta entrega, condições exclusivas',
    ],
  },
  {
    nome: 'Air Ski Brave',
    ...
    tagline: 'Tensão calibrada, corda que não estoura.',
    emoji: '⛷️',
    alias: 'skierg',
    features: [
      'Puxador em borracha no padrão C2',
      'Painel com sensor p1, trabalho em metragem confiável',
      'Corda interna estendida — não estoura fácil',
      'Tensão de puxada com calibragem Brave',
      'Plataforma inclusa',
      'Pronta entrega',
    ],
  },
  {
    nome: 'Escada Ergométrica Brave',
    badge: 'Lançamento',
    badgeCls: 'bg-purple-500',
    tagline: 'Lançamento exclusivo com frete grátis.',
    emoji: '🪜',
    alias: 'escada',
    features: [
      ...
      <section className="relative overflow-hidden py-24 px-5">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          <span className="inline-block text-orange-400 text-[11px] font-black tracking-[0.2em] uppercase border border-orange-400/30 bg-orange-400/8 px-4 py-1.5 rounded-full mb-5">
            Linha Ergômetros 2025
          </span>
          <h1 className="text-5xl md:text-7xl font-black leading-[0.95] tracking-tight mb-6">
            Ergômetros Profissionais<br />
            <span className="text-orange-400">para o seu Box</span>
          </h1>
          <p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
            Equipamentos desenvolvidos para suportar os treinos mais intensos,
            projetados com tecnologia de ponta e o melhor custo-benefício do mercado.
          </p>
          <a
            href={wa(MSG_GERAL)}
            target="_blank"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {PRODUTOS.map((produto) => {
              ...
              return (
                <div
                  key={produto.alias}
                  className="bg-[#111111] border border-white/6 rounded-2xl overflow-hidden flex flex-col hover:border-orange-500/30 transition-all group"
                >
                  {/* Imagem placeholder */}
                  <div className="relative h-52 bg-gradient-to-br from-white/3 to-transparent flex items-center justify-center border-b border-white/5">
                    <span className={`absolute top-3 left-3 text-[11px] font-black text-white px-2.5 py-1 rounded-full ${produto.badgeCls}`}>
                      {produto.badge}
                    </span>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
            ...
            <div className="flex items-center gap-3">
              {[
                ...
                { href: wa(MSG_GERAL), Icon: MessageCircle },
              ].map(({ href, Icon }, i) => (
                <a
                  key={i}
                  href={href}
